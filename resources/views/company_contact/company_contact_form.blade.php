...
@section('content')
    <div class="form-container content-width-full company_contact-form-content js-ak-delete-container">
        <form method="POST"
              action="@isset($data->id){{route("company_contact.update", $data->id)}}@else{{route("company_contact.store")}}@endisset"
            ...
            <div class="form-header">
                ...
                @can('delete-CompanyContact')
                    <div class="form-delete-record">
                        @if(isset($data->id))
                            <a href="#" data-link="{{route('company_contact.destroy',$data->id)}}"
            ...
            <div class="form-content">
                        ...
                        <div class="input-data">
                            <select name="company_id" id="company_id" class="form-select">
                                <option value="">Select Company</option>@foreach ($company_list as $list)
                                    <option
                                            value="{{$list->id}}" {{ $data->company_id == $list->id ? "selected" : "" }}>{{ $list->company_name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="row-33">
                    <div class="input-container">
                        <div class="input-label">
                            <label for="company_address_id">Address Type <span class="required">*</span></label>
                        </div>
                        <div class="input-data">
                            <select name="company_address_id" id="company_address_id" class="form-select">
                                <option value="">Select Address Type
                                </option>
                ...
                <div class="row-25">
                    <div class="input-container">
                        <div class="input-label">
                            <label for="phone_one">Contact No One <span class="required">*</span></label>
                        </div>
                        <div class="input-data">
                ...
                <div class="row-25">
                    <div class="input-container">
                        <div class="input-label">
                            <label for="phone_two">Contact No Two </label>
                        </div>
                        <div class="input-data">
...
@push('bottom_script')
    <script>
        ...
        $(document).ready(function () {
            const companySelector = '#company_id';
            const addressTypeSelector = '#company_address_id';


            // Populate State dropdown based on selected Country
            $(companySelector).on('change', function () {
                loadDropdownData(
                    "{{ route('get_company_address_by_company') }}",
                    {company_id: $(this).val()},
                    addressTypeSelector
                );
            });
